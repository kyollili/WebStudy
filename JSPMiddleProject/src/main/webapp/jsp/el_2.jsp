...
<%--
		EL의 연산자 (자바가 아니다)
		1) 산술 연산자
			+ : 순수하게 산술 연산만 가능 (문자열 결합(x)) => +=으로 문자열 결합
			- 
			*  
			/ : 정수/정수=실수		div
				$ {10/5} = 2
...
<body>
<h1>EL 연산자 (산술연산자 + , - , * , /(div) , %(mod)): 문자열 결합은 +=, null은 자동으로 0</h1>
<%-- &#36;{10+10)=${10+10 }<br>
&#36;{"10"+10}=${"10"+10 }<br>
&#36;{"10 "+10}=${"10 "+10 }<br>  공백이 들어가면 오류 남
&#36;{null+100}=${null+100 }<br> null은 0으로 취급 
...
<h1>비교연산자 (결과: true/false ==> 조건문 사용 시)</h1>
<h1>비교연산자 종류 (==(equal), !=(ne) , <(lt) , >(gt) , <=(le) , >=(ge))</h1>
...
<h1>삼항연산자 (조건?값1:값2)=ㅑif~else => 조건 true:값1 , false:값2 => page변경</h1>
<%--&#36; {(10==10)?'A':'B'}=${(10==10)?'A':'B' }
