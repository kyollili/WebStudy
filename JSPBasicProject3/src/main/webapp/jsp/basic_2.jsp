<%--
		...
		1. 사용자 브라우저 주소창에서 요청 (사용자 요청은 주소창에서 한다) => 주소창은 반드시 파일명을 마지막에 첨부
			http://localhost/JSPBasicProject3/jsp/basic_.jsp
							---------------------------------- basic_2.jsp를 브라우저에서 보여달라
			*** 주소창 ==> 서버가 받아서 처리
		2. Web Server에서 파일을 받아서 (.html, .xml)이 아닌 경우 => 톰캣으로 전송
									-------------
		3. 톰캣
			1) basic_2.jsp ==> 자바로 변경 (basic_2.jsp.java) => 서블릿(웹에서 실행하는 자바 파일)
		...
		=======================================================================================
			_jspInit() ==> web.xml (설정)
			_jspService() ==> 사용자 요청에 의한 HTML 작성 =====> 메소드 안에 들어가는 소스 코딩을 한다 (JSP 파일)
			_jspDestroy() ==> 메모리 해제
