 ...
 * 		---
 * 		Client => 명소 번호 전송
 * 		Server => 번호에 해당되는 모든 데이터 읽어서 화면 변경
 * 
 * 		게시판 ==> 글쓰기
 * 			Client => 글쓴 내용을 전송
 ...
 * 	  -----------------------------------------------
 * 	2) 서버에서 ==> WAS에서 서블릿 클래스를 찾는다 (HelloServelt)
 * 				  --- WebApplication Server(톰캣, 레진..) => WAS, 형상관리(깃), Container(Spring), Component(자바 클래스)
 * 	3) init() 메소드 호출
...
public class HelloServlet extends HttpServlet {
	 ...
	 * 														 ----------------
	 * 															main.jsp로 no 값을 전송 => 메소드가 없기 때문에 매개변수를 이용할 수 없다
	 *				=> 단순한 검색어, 페이지 번호 전송 ....(보안과 관련이 없는 데이터)
	 *			POST : no=10 ==> 감춰서 보내준다(URL 뒤에 데이터가 안 보인다)
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//									HTML		XML			JSON
		response.setContentType("text/html;charset=UTF-8"); //한글 포함
		// 2. 어떤 브라우저로 전송할지 설정
		...
		System.out.println("</body>");
		System.out.println("</html>");*/ //system이 있으면 서버 안에서 출력 없으면 브라우저에 출력
		out.println("<html>");
		out.println("<body>");
