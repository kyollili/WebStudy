 ...
 * 						연결에 소요되는 시간을 줄일 수 있다 (속도 빠르다), Connection의 누수 현상을 방지할 수 있다
 * 						재사용이 가능 (사용 => 반환)
 * 				=> Setting : server.xml =>
 * 					<Resource
 ...
 * 						password=""
 * 						------------------ Connection 생성
 * 						name="jdbc/oracle" ==> Connection 객체를 찾기 위한 구분자 => lookup() : 이름으로 객체 주소 열기
 * 						type="javax.sql.DataSource" => 얻어온 주소값을 어떤 클래스로 받을지 설정
 * 						-----------
 * 							maxActive="" : 최대 Connection 생성 개수 
 * 							maxIdle="" : 실제 POOL에 저장된 개수
 * 							maxWait="-1" : Connection을 전부 사용시에는 사용자가 반환할 때까지 대기 (대기시간)
 * 							------------ 초, 음수면 무한정
 * 					/>
...
public class DataBoardDAO {
	...
	private PreparedStatement ps; //오라클로 SQL 문장 전송 / 값을 가지고 온다
	 ...
	 * 			2) 결과값을 어떤 값 => 리턴형
	 * 				= 목록(ArrayList)
	 * 				= 상세 (~VO) => 게시물 한 개에 대한 모든 데이터변수를 가지고 있다
	 * 				= 일반 (총페이지, 검색, 로그인처리)
	 * 					경우의 수
	 * 					------- 여러 개 (String, int) => ID,PWD,OK
	 * 					------- 한 개 해당 데이터를 사용
	 * 			3) 메소드를 구현
	 * 
	...
	public ArrayList<DataBoardVO> databoardListData(int page)
	{
		...
		try
		{
			//1. Connection 주소 얻기
			getConnection();
			//2. SQL 문장 ==> index => UNIQUE, PRIMARY KEY => 자동으로 만들어진다
			...
			while(rs.next())
			{
				...
				vo.setDbday(rs.getString(4)); //TO_CHAR 나오면 dbday 추가해서 문자열로 받기
			...
			rs.close();
			// =====================> 라이브러리화 한 개의 메소드로 통일이 가능 : selectList(SQL) ==> MyBatis
		}catch(Exception ex)
		{
			ex.printStackTrace();
		}
